 ...
 *
 * Factors:
 *  - Historical no-show/cancellation rate (0-40 pts)
...
export function computeCancellationRisk(appointmentId: number): RiskScore {
  ...
  if (!appt) return { score: 0, factors: { noShowRate: 0, timeSinceBooking: 0, dayOfWeek: 0, daysSinceLastVisit: 0, cancellationCount: 0 } };

  const clientId = appt.client_id;

  // ── Factor 1: Historical no-show/cancel rate (0-40 pts) ────────────────────
  const history = rawSqlite.prepare(`
    SELECT
      COUNT(*) AS total,
      SUM(CASE WHEN status IN ('CANCELLED', 'NO_SHOW') THEN 1 ELSE 0 END) AS bad
    FROM appointments
    WHERE client_id = ? AND id != ? AND status IN ('COMPLETED', 'CANCELLED', 'NO_SHOW')
  `).get(clientId, appointmentId) as { total: number; bad: number };

  let noShowRatePts = 0;
  if (history.total >= 3) {
    const rate = history.bad / history.total;
    noShowRatePts = Math.round(rate * 40);
  } else if (history.total > 0) {
    // Low data — partial penalty
    const rate = history.bad / history.total;
    noShowRatePts = Math.round(rate * 20);
  }
  ...
  const totalCancels = rawSqlite.prepare(`
    ...
    WHERE client_id = ? AND status IN ('CANCELLED', 'NO_SHOW') AND id != ?
  ...
  const total = Math.min(100, noShowRatePts + timeSinceBookingPts + dayOfWeekPts + daysSinceLastVisitPts + cancellationCountPts);

  return {
    score: total,
    factors: {
      noShowRate: noShowRatePts,
