...
const seed = () => {
  ...
  for (let i = 1; i <= 14; i++) {
    ...
    if (i % 3 === 0) appt(8, 4, 5, 3, d(-i, 14), d(-i, 14, 50), i % 6 === 0 ? "CANCELLED" : "COMPLETED", 1500);
  }

  // PAST — CANCELLED
  appt(5, 3, 1, 1, d(-4, 15), d(-4, 16), "CANCELLED", 1200);
  ...
  sqlite.prepare(`
    ...
    (5, 'LATE_CANCEL', 'Pozdní zrušení dne ${new Date(Date.now() - 4 * 86400000).toISOString().slice(0, 10)}', -10),
