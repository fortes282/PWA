...
export default function EmployeeHomework() {
  ...
  const removeExercise = (i: number) => { haptics.medium(); setExercises(exercises.filter((_, idx) => idx !== i)); };
        ...
        <div className="max-w-3xl mx-auto space-y-6">
                          ...
                          {mediaFiles.map((mf, idx) => (
                            <div key={idx} className="relative group">
                              ...
                              <motion.button
                                ...
                                whileTap={shouldReduce ? undefined : { scale: 0.85 }}
                                transition={{ type: "spring", stiffness: 500, damping: 22 }}
                                onClick={() => { haptics.medium(); setMediaFiles((prev) => prev.filter((_, i) => i !== idx)); }}
          ...
          <AnimatePresence>
            {!homework || homework.length === 0 ? (
              ...
              >
                <p className="text-gray-500 dark:text-gray-400">Zatím žádná přiřazená cvičení</p>
              </motion.div>
            ) : (
              <div
                key="list"
