...
export async function assertNoTextClipping(page: Page, label: string) {
  ...
  const clipped = await page.evaluate(() => {
    ...
    for (const el of candidates) {
      ...
      // Only check very large stat values (text-3xl+) — smaller sizes clip legitimately on narrow cards
      const isBigStat = typeof cls === "string" && (cls.includes("text-3xl") || cls.includes("text-4xl"));
